...
PImage [] efectoRadioVerde = new PImage [cantImg];
PImage [] efectoRadioAmarillo = new PImage [cantImgA];
PImage [] efectoRadioRojo = new PImage [cantImgR];
String [] textosPerder, textosGanar; //estado perder-ganar
boolean botonPresionado;
PImage fondoInicio, tituloInicio, tituloPerder, tituloGanar, instrucciones, botonInicio, botonJugar, botonReintentar, bailarines, radio, fondoJugando, fondoJugando2;
String [] nombre={"frag00", "frag01", "frag02", "frag03", "frag04", "frag05", "frag06", "frag07", "frag02", "frag08", "frag09", "frag10", "frag11"};
PImage[] barra;
PImage barra1[];
PImage[] barraColor;
FWorld mundo;
...
void setup() {
  ...
  numNota=0;
  barra1[0] = loadImage("imagenes/barraGris.png");
  barra1[1] = loadImage("imagenes/barraGris1.png");
  pizarron= loadImage("imagenes/pizarronEmpanadas.jpg");
  //////Efecto radio
  for (int i = 0; i <efectoRadioVerde.length; i++) {
    efectoRadioVerde[i] = loadImage("sonidoVerde0" + i + ".png" );
    efectoRadioVerde[i].resize(220,220);
  }
  for (int i = 0; i <efectoRadioAmarillo.length; i++) {
    efectoRadioAmarillo[i] = loadImage("sonidoAmarillo0" + i + ".png" );
    efectoRadioAmarillo[i].resize(250,250);
  }
  for (int i = 0; i <efectoRadioRojo.length; i++) {
    efectoRadioRojo[i] = loadImage("sonidoRojo0" + i + ".png" );
    efectoRadioRojo[i].resize(250,250);
  }
  int tamText = 10;
  fuenteTexto = createFont("PatrickHand-Regular.ttf", tamText);
  ...
  for (int i=0; i<4; i++) {
    ...
    empanadas[i]= loadImage("imagenes/empanadasPizarron"+ i +"V2.png");

  }

  for (int i=0; i<10; i++) {

    barra[i]= loadImage("imagenes/barraProgreso"+ i +".png");
  }
  for (int i=0; i<10; i++) {

    barraColor[i]= loadImage("imagenes/nota"+ i +".png");
  }
  /*empanadas[3].resize(140, 60);
   empanadas[2].resize(92, 60);
   empanadas[1].resize(44, 59);*/
  empanadas[3].resize(200, 100);
  empanadas[2].resize(140, 100);
  empanadas[1].resize(70, 100);
  amorSalvaje.loop();
  ganar.amp(0.2);
  //imagenes
  fondoInicio = loadImage("imagenes/fondoInicio_fullScreen.jpg");
  tituloInicio = loadImage("imagenes/tituloAentro.png");
  tituloInicio.resize(800,196);
  tituloPerder = loadImage("imagenes/tituloPerdiste.png");
  tituloPerder.resize(800,213);
  tituloGanar = loadImage("imagenes/tituloGanaste.png");
  tituloGanar.resize(796,213);
  instrucciones = loadImage("imagenes/instrucciones.png");
  instrucciones.resize(500,145);
  ...
  fondoJugando2 = loadImage("imagenes/fondo_jugando_fullScreen.jpg");
  //textos
  ...
  meta.setNoFill();
  caja.setNoFill();
  mundo.setEdges(0, 0, width, height+100);
...
void draw() {
  ...
  if (estado==1) {
    ...
    //fondoInicio.resize(1920, 1080);
    image(botonJugar, width*0.43, height*0.71);
    image(tituloInicio, width/2-400, height*0.15);
    image(instrucciones, width*0.36, height*0.4);
    ganar.stop();
...
marcaDeTiempo2=0;
  ...
  if (estado==2) {
    ...
    if (numeroDeFragmento2>9) {
      numeroDeFragmento2=0;
    }
   ...
   // println(cambia);
    ...
    meta.setNoStroke();
    meta.setPosition(1550, 900); //rectangulo bailarines
   ...
   // textSize(20);
    ...
    image(fondoJugando2, 0, 0);
    //image(fondoJugando, 0, 0);
    //fondoJugando.resize(1920, 1080);
    fill(#FCFAFA);
    ...
    mundo.draw();
    if (opo==3) {
      image(efectoRadioVerde[indexImg], 310, 380);
      indexImg = (indexImg+1) % efectoRadioVerde.length;
    }

    if (opo==2) {
      image(efectoRadioAmarillo[indexImg], 330, 400);
      indexImg = (indexImg+1) % efectoRadioAmarillo.length;
      
    }

    if (opo==1) {
      image(efectoRadioRojo[indexImg], 330, 400);
      indexImg = (indexImg+1) % efectoRadioRojo.length;
    }
    gauchos[cambia].resize(700, 600);

    image(gauchos[cambia], px, 500);// acá esta
    if (opo==3) {
      cambia=etapaUno;
      if (++pf >8) pf =0;
      cambia =pf;
    ...
    //     image(barra1[0], 340, 20);
    //   }
    //   if (entroFragmento[0]==false) {
    //     image(barra1[1], 340, 20);
    //   }
    ...
    //   }
    if (entroFragmento[0]==false) {
      image(barra[0], 320, 20);
    }
    if (entroFragmento[1]==false) {
      image(barra[1], 440, 19);
    }
    if (entroFragmento[2]==false) {
      image(barra[2], 548, 19);
    }
    if (entroFragmento[3]==false) {
      image(barra[3], 640, 20);
    }
    if (entroFragmento[4]==false) {
      image(barra[4], 720, 20);
    }
    if (entroFragmento[5]==false) {
      image(barra[5], 800, 20);
    }
    if (entroFragmento[6]==false) {
      image(barra[6], 900, 20);
    }
    if (entroFragmento[7]==false) {
      image(barra[7], 990, 20);
    }
    if (entroFragmento[8]==false) {
      image(barra[8], 1100, 19);
    }
    if (entroFragmento[9]==false) {
      image(barra[9], 1180, 19);
    }

    //Barra pintada, cambia segun el fragmento que entre
    if (entroFragmento[0]==true) {
      //image(barraColor[1], 340, 20);
      //barraColor[1].resize(75,75);
      push();
      tint(255, 199, 84);
      image(barra[0], 320, 20);
    ...
    image(pizarron, 1460, 5);
    pizarron.resize(280, 140);
    image(empanadas[opo], 1500, 20);
    ...
    if (progreso>=15 || entroFragmento[0] && entroFragmento[1] && entroFragmento[2] && entroFragmento[3] && entroFragmento[4] && entroFragmento[5] && entroFragmento[6] && entroFragmento[7] && entroFragmento[8] && entroFragmento[9] ) {
      estado=4;
      ganar.play();
  ...
  if (estado==3) {
    ...
    image(tituloPerder, width/2-400, height*0.15);
  ...
  if (estado==4) {
    ...
    image(tituloGanar, width/2-400, height*0.15);
  ...
  println(numeroDeFragmento, entroFragmento[0], entroFragmento[2],entroFragmento[3], entroFragmento[4], entroFragmento[5], entroFragmento[6], entroFragmento[7], entroFragmento[8], entroFragmento[9]);
}
...
void botonCustom(String textoB, int queEstado, int x, int y, int posx, int posy) {
  ...
  if (mouseX > x && mouseX < posx +x && mouseY > y && mouseY < posy + y ) {
    if (mousePressed==true && contador>=50) {
      ...
      mundo.add(borde);
      mundo.add(bordeA);
      mundo.setEdges(10, 0, width, height+100);
      opo=3;
    }
  }
